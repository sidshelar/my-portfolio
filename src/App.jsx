...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
const ADMIN_PASSWORD = "siddhant2025"; // ← change this to whatever you want
  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    label {
      ...
      color: ${T.muted}; display: block; margin-bottom: 6px;
      font-family: 'DM Sans', sans-serif;
    }
    ...
    .admin-badge {
      ...
      color: rgba(255,200,0,0.7); font-size: 10px; letter-spacing: 0.12em;
...
function Nav({ onBack, showBack, onAddProject, isAdmin, onAdminToggle }) {
  ...
  return (
    <nav style={{
      ...
      >
        <span className="display" style={{ fontSize: 15, color: T.text, letterSpacing: "0.01em" }}>
          Siddhant Shelar
        </span>
      </button>
      ...
      <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
        {showBack ? (
          ...
          <>
            ...
            {isAdmin && (
              <button className="btn-ghost" onClick={onAddProject} style={{ marginLeft: 4 }}>
                + Add Project
              </button>
            )}
            <button
              ...
              className="nav-link"
              style={{ color: isAdmin ? "rgba(255,200,0,0.6)" : T.muted, marginLeft: 8 }}
            >
              {isAdmin ? "● Admin" : "○"}
            </button>
          </>
  ...
  return (
    <section id="projects" style={{ padding: "0 48px 120px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 48 }}>
        <div>
          <p style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: T.muted, marginBottom: 12 }}>
            Selected Work
          </p>
          <h2 className="display" style={{ fontSize: "clamp(36px, 5vw, 72px)", color: T.text, letterSpacing: "-0.01em" }}>
            Projects.
          </h2>
        </div>
        {isAdmin && (
      ...
      {loading ? (
        <div style={{ padding: "80px 0", textAlign: "center", color: T.muted, fontSize: 12, letterSpacing: "0.1em" }}>
          Loading projects...
        </div>
      ) : projects.length === 0 ? (
      ...
      {isAdmin && (
        <div style={{ marginTop: 40, display: "flex", justifyContent: "center" }}>
          <button className="btn-ghost" onClick={onAdd} style={{ padding: "14px 48px" }}>
            + Add Project
          </button>
        </div>
      )}
...
function ProjectRow({ project, index, onClick, isAdmin, onEdit, onDelete }) {
  ...
  return (
    <div className="project-row" onClick={onClick} style={{
      padding: "28px 0",
      display: "grid",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
        ...
        <div>
          <h3 className="display" style={{ fontSize: "clamp(18px, 3vw, 36px)", color: T.text, lineHeight: 1, letterSpacing: "-0.01em" }}>
...
// ─── PROJECT DETAIL ───────────────────────────────────────────────────────────
function ProjectDetail({ project, onClose, projects, onOpenProject, isAdmin, onEdit }) {
  ...
  return (
    <div className="detail-page" ref={ref}>
      ...
      <div style={{ paddingTop: 60 }}>
        {media.length > 0 ? (
          <div style={{ width: "100%", maxHeight: "78vh", overflow: "hidden", background: T.bg2 }}>
            {media[0].type === "video"
              ? <video src={media[0].url} autoPlay loop muted playsInline style={{ width: "100%", maxHeight: "78vh", objectFit: "cover", display: "block" }} />
              : <img src={media[0].url} alt={project.title} style={{ width: "100%", maxHeight: "78vh", objectFit: "cover", display: "block" }} />
            }
          </div>
        ) : (
          <div style={{ height: "35vh", background: T.bg2 }} />
        )}
      </div>

      {/* Title block */}
      <div style={{ padding: "64px 48px 0", maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "start" }}>
          <div>
            {project.tag && (
              <p style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: T.muted, marginBottom: 16 }}>{project.tag}</p>
            )}
            <h1 className="display" style={{ fontSize: "clamp(48px, 8vw, 108px)", lineHeight: 0.9, letterSpacing: "-0.01em", color: T.text }}>
              {project.title || "Untitled"}.
            </h1>
            {isAdmin && (
              <button onClick={() => onEdit(project)} className="btn-ghost" style={{ marginTop: 24, fontSize: 10 }}>Edit Project</button>
            )}
          </div>
        ...
        <div style={{
          ...
          gap: 80, marginTop: 64, paddingBottom: 64,
          borderBottom: `1px solid ${T.border}`,
        }}>
          <div>
            {project.description && (
              <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", maxWidth: 500 }}>
                {project.description}
              </p>
            )}
            {project.link && (
              <a href={project.link} target="_blank" rel="noopener noreferrer" style={{
                ...
                color: T.text, textDecoration: "underline", textUnderlineOffset: 5,
              }}>
      ...
      {media.length > 1 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 3, marginTop: 3 }}>
          {media.slice(1).map((m, i) => (
            <div key={i} style={{ width: "100%", background: T.bg2 }}>
              {m.type === "video"
                ? <video src={m.url} controls playsInline style={{ width: "100%", display: "block", maxHeight: "85vh", objectFit: "contain" }} />
        ...
        <div style={{
          ...
          <div>
            ...
            <button onClick={() => onOpenProject(nextProject)} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
              <h3 className="display" style={{ fontSize: "clamp(28px, 4vw, 56px)", color: T.text, letterSpacing: "-0.01em" }}>
                {nextProject.title} →
              </h3>
            </button>
          </div>
...
function ProjectModal({ onClose, onSave, initial }) {
  ...
  const [form, setForm] = useState(initial || {
    title: "", tag: "", duration: "", year: new Date().getFullYear().toString(),
    description: "", skills: "", link: "", link_label: "", media: [],
  });
  ...
  const processFiles = useCallback((files) => {
    ...
    Promise.all(tasks).then(newMedia => {
      ...
      <div className="modal-box">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <div>
            ...
            <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="e.g. Stress Analysis Rig" />
          </div>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => set("description", e.target.value)} placeholder="What did you build? What problem does it solve?" rows={4} style={{ resize: "vertical" }} />
          </div>

          <div>
            <label>Methods / Skills Used</label>
            <input value={form.skills} onChange={e => set("skills", e.target.value)} placeholder="FEA, SolidWorks, ASTM D638, ..." />
          </div>
          ...
          <div>
            ...
            <div
              ...
              style={{
                border: `1px dashed ${dragOver ? T.borderHover : T.border}`,
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 5, marginTop: 10 }}>
                {form.media.map((m, i) => (
                  <div key={i} style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden", background: T.bg2 }}>
                    ...
                    {i === 0 && (
                      <div style={{ position: "absolute", top: 3, left: 3, background: "rgba(0,0,0,0.75)", color: T.text, fontSize: 8, letterSpacing: "0.1em", textTransform: "uppercase", padding: "2px 5px" }}>Cover</div>
                    )}
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 360, padding: 36 }}>
        ...
        <input
          ...
          style={{ borderColor: err ? "#e05555" : T.border }}
          autoFocus
        />
        {err && <p style={{ fontSize: 11, color: "#e05555", marginTop: 8, letterSpacing: "0.05em" }}>Incorrect password</p>}
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 380, padding: 36 }}>
        ...
        <p style={{ fontSize: 13, color: T.muted, lineHeight: 1.7, marginBottom: 28 }}>
          This will permanently delete <strong style={{ color: T.text }}>{project.title}</strong> and all its media. This cannot be undone.
        </p>
  ...
  return (
    <section id="contact" style={{ padding: "120px 48px 80px", borderTop: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <h2 className="display" style={{ fontSize: "clamp(40px, 8vw, 100px)", color: T.text, lineHeight: 0.9, letterSpacing: "-0.01em", marginBottom: 80 }}>
          Let's connect.
        </h2>
...
export default function App() {
  ...
  // Browser back from home (pop any leftover state)
  useEffect(() => {
    const handler = (e) => {
      if (!e.state?.projectId) setActiveProject(null);
    };
    window.addEventListener("popstate", handler);
    return () => window.removeEventListener("popstate", handler);
  }, []);
  ...
  const handleAddProject = async (form) => {
    const row = {
      ...
      title: form.title, tag: form.tag, duration: form.duration,
      year: form.year, description: form.description, skills: form.skills,
  ...
  return (
    <>
      ...
      {activeProject ? (
        ...
        <div style={{ background: T.bg, minHeight: "100vh" }}>
          <Nav
            ...
            isAdmin={isAdmin}
            onAdminToggle={() => isAdmin ? setIsAdmin(false) : setShowAdminModal(true)}
          />
      ...
      {showAddModal && isAdmin && (
        <ProjectModal onClose={() => setShowAddModal(false)} onSave={handleAddProject} />
      )}
      {editingProject && isAdmin && (
        <ProjectModal initial={editingProject} onClose={() => setEditingProject(null)} onSave={(form) => handleEditProject({ ...form, id: editingProject.id })} />
      )}
